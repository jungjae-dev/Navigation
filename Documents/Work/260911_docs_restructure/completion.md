...
4. **Work 커밋 재개**: 260501~260911 20폴더(본 작업 폴더 포함)를 시기별 3커밋으로 (문서·ref 이미지만, 원시 데이터 자동 필터)
...
## 리뷰 반영 (PR #50 Opus·Sonnet 교차 리뷰)

HIGH 0건. MED 4건 전부 수정: constitution의 미실재 `Theme.Palette`·"이탈 쿨다운" 정정(메모리 문구를 코드 검증 없이 승격한 것이 원인 — "코드에 실재할 때만" 규칙의 필요성을 스스로 입증), 121장소 shapefile 출처·재생성 방법을 build_hotspot_areas.py에 기록, `.qmd` 등 QGIS 사이드카 ignore 추가·기커밋분 제거. LOW: plan.md 상대링크(동결 전 수정), data_pipeline.md 현행 위치 배너, README 정본 표시·번들 배열 형식 문서화, MapMatcher 주석 사용처 보강. 260508 문서의 도로 위 좌표 1점은 로그 예시로 유지(판단 기록).
